...
(function () {
    ...
    function setupEventListeners() {
        ...
        // Listener para toggle de notificaciones de errores (SMTP)
        const errorNotificationsEnabled = document.getElementById('error-notifications-enabled');
        errorNotificationsEnabled?.addEventListener('change', (e) => {
    ...
    async function fetchCompanySettings() {
        try {
            ...
            // Cargar configuración de notificaciones de errores y SMTP
            const errorNotificationsEnabledInput = document.getElementById('error-notifications-enabled');
            if (errorNotificationsEnabledInput) {
                errorNotificationsEnabledInput.checked = data.error_notifications_enabled || false;
                errorNotificationsEnabledInput.dispatchEvent(new Event('change'));
            }

            const notificationEmailInput = document.getElementById('notification-email');
            if (notificationEmailInput && data.notification_email) {
                notificationEmailInput.value = data.notification_email;
            }

            // Cargar configuración SMTP si existe
            if (data.smtp_config) {
                const smtpConfig = typeof data.smtp_config === 'string' ? JSON.parse(data.smtp_config) : data.smtp_config;
                ...
                if (smtpHostEl && smtpConfig.host) smtpHostEl.value = smtpConfig.host;
                if (smtpPortEl && smtpConfig.port) smtpPortEl.value = smtpConfig.port;
                if (smtpUserEl && smtpConfig.user) smtpUserEl.value = smtpConfig.user;
                if (smtpPasswordEl && smtpConfig.password) smtpPasswordEl.value = smtpConfig.password;
                if (smtpFromEmailEl && smtpConfig.from_email) smtpFromEmailEl.value = smtpConfig.from_email;
                if (smtpFromNameEl && smtpConfig.from_name) smtpFromNameEl.value = smtpConfig.from_name;
                if (smtpSecureEl) smtpSecureEl.checked = smtpConfig.secure !== false; // default true
            }
    ...
    async function performSettingsSave(button, options = {}) {
        ...
        try {
            ...
            // Construir objeto de configuración SMTP
            const errorNotificationsEnabledEl = document.getElementById('error-notifications-enabled');
            const smtpConfig = errorNotificationsEnabledEl?.checked ? {
                ...
                from_name: document.getElementById('smtp-from-name')?.value?.trim() || 'ELINA IA'
            } : null;

            // Agregar campos de notificaciones a settingsData
            settingsData.error_notifications_enabled = errorNotificationsEnabledEl?.checked || false;
            settingsData.notification_email = document.getElementById('notification-email')?.value?.trim() || null;
            settingsData.smtp_config = smtpConfig;
