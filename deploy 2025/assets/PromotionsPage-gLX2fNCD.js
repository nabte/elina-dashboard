import { j as jsxRuntimeExports, b as reactExports } from './landing-DMNKFuas.js';
import { u as useServices } from './MainLayout-C0Ev0ilC.js';
import { e as useNotification, u as useAuth } from './index-B4F2XZYo.js';
import { f as formatCurrency } from './formatting-BolSclPB.js';
import { P as ProductPickerModal } from './ProductPickerModal-BgJ2gb1c.js';
import ConfirmationModal from './ConfirmationModal-D0s46AqS.js';
import './inventoryCountService-Bo8nZII3.js';
import './SettingsIcon-DbPfflvT.js';
import './vendor-DeEp8S9j.js';
import './supabase-vendor-DZiv2hl9.js';

const PromotionsTable = ({ promotions, isLoading, onEditPromotion, onDeletePromotion }) => {
  if (isLoading) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "bg-slate-800 rounded-lg p-8 text-center text-slate-400", children: "Cargando promociones..." });
  }
  if (promotions.length === 0) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "bg-slate-800 rounded-lg p-8 text-center text-slate-400", children: "No hay promociones creadas. Crea una nueva para comenzar." });
  }
  const getPromotionTypeLabel = (type, value) => {
    switch (type) {
      case "2x1":
        return "2x1";
      case "3x2":
        return "3x2";
      case "percentage":
        return `${value}% desc.`;
      case "fixed_discount":
        return `${formatCurrency(value)} desc.`;
      default:
        return type;
    }
  };
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "bg-slate-800 rounded-lg overflow-hidden", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "overflow-x-auto", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("table", { className: "w-full", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("thead", { className: "bg-slate-700", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("tr", { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "px-4 py-3 text-left text-sm font-semibold text-slate-200", children: "SKU" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "px-4 py-3 text-left text-sm font-semibold text-slate-200", children: "Nombre" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "px-4 py-3 text-left text-sm font-semibold text-slate-200", children: "Tipo" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "px-4 py-3 text-left text-sm font-semibold text-slate-200", children: "Componentes" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "px-4 py-3 text-left text-sm font-semibold text-slate-200", children: "Precio Original" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "px-4 py-3 text-left text-sm font-semibold text-slate-200", children: "Precio Promo" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "px-4 py-3 text-left text-sm font-semibold text-slate-200", children: "Margen" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "px-4 py-3 text-left text-sm font-semibold text-slate-200", children: "Calendario" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "px-4 py-3 text-left text-sm font-semibold text-slate-200", children: "Estado" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "px-4 py-3 text-left text-sm font-semibold text-slate-200", children: "Acciones" })
    ] }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("tbody", { className: "divide-y divide-slate-700", children: promotions.map((promotion) => {
      const isScheduled = promotion.isScheduled && promotion.startDate && promotion.endDate;
      const now = /* @__PURE__ */ new Date();
      const isActiveNow = isScheduled ? new Date(promotion.startDate) <= now && new Date(promotion.endDate) >= now : promotion.isActive;
      return /* @__PURE__ */ jsxRuntimeExports.jsxs("tr", { className: "hover:bg-slate-700/50", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3 text-sm text-slate-300", children: promotion.sku }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3 text-sm text-slate-300 font-medium", children: promotion.name }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3 text-sm text-slate-300", children: getPromotionTypeLabel(promotion.promotionType, promotion.promotionValue) }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("td", { className: "px-4 py-3 text-sm text-slate-400", children: [
          promotion.components.length,
          " ",
          promotion.components.length === 1 ? "componente" : "componentes"
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3 text-sm text-slate-400", children: formatCurrency(promotion.originalPrice) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3 text-sm text-slate-300 font-semibold text-green-400", children: formatCurrency(promotion.finalPrice) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3 text-sm", children: promotion.profitMargin !== void 0 ? /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: promotion.profitMargin >= 0 ? "text-green-400" : "text-red-400", children: [
          promotion.profitMargin.toFixed(1),
          "%"
        ] }) : /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-slate-500", children: "-" }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3 text-sm text-slate-400", children: isScheduled ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-xs", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { children: new Date(promotion.startDate).toLocaleDateString() }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-slate-500", children: "hasta" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { children: new Date(promotion.endDate).toLocaleDateString() })
        ] }) : /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-slate-500", children: "Sin calendario" }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3 text-sm", children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `px-2 py-1 rounded-full text-xs font-semibold ${isActiveNow ? "bg-green-900/50 text-green-300" : "bg-slate-700 text-slate-400"}`, children: isActiveNow ? "Activa" : "Inactiva" }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3 text-sm", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-2", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              onClick: () => onEditPromotion(promotion),
              className: "text-sky-400 hover:text-sky-300 transition-colors",
              children: "Editar"
            }
          ),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              onClick: () => onDeletePromotion(promotion),
              className: "text-red-400 hover:text-red-300 transition-colors",
              children: "Eliminar"
            }
          )
        ] }) })
      ] }, promotion.id);
    }) })
  ] }) }) });
};

const PromotionFormModal = ({ isOpen, onClose, onSubmit, initialData }) => {
  const { promotionService } = useServices();
  const { addNotification } = useNotification();
  const [sku, setSku] = reactExports.useState("");
  const [name, setName] = reactExports.useState("");
  const [promotionType, setPromotionType] = reactExports.useState("percentage");
  const [promotionValue, setPromotionValue] = reactExports.useState("");
  const [finalPrice, setFinalPrice] = reactExports.useState("");
  const [isActive, setIsActive] = reactExports.useState(true);
  const [isScheduled, setIsScheduled] = reactExports.useState(false);
  const [startDate, setStartDate] = reactExports.useState("");
  const [endDate, setEndDate] = reactExports.useState("");
  const [applyToSeparateProducts, setApplyToSeparateProducts] = reactExports.useState(false);
  const [components, setComponents] = reactExports.useState([]);
  const [isLoading, setIsLoading] = reactExports.useState(false);
  const [isProductPickerOpen, setIsProductPickerOpen] = reactExports.useState(false);
  const [activeComponentIndex, setActiveComponentIndex] = reactExports.useState(null);
  const [originalPrice, setOriginalPrice] = reactExports.useState(0);
  const [suggestedPrice, setSuggestedPrice] = reactExports.useState(0);
  const [costPrice, setCostPrice] = reactExports.useState(0);
  const [profitMargin, setProfitMargin] = reactExports.useState(null);
  reactExports.useEffect(() => {
    if (isOpen) {
      if (initialData) {
        setSku(initialData.sku);
        setName(initialData.name);
        setPromotionType(initialData.promotionType);
        setPromotionValue(initialData.promotionValue.toString());
        setFinalPrice(initialData.finalPrice.toString());
        setIsActive(initialData.isActive);
        setIsScheduled(initialData.isScheduled);
        setStartDate(initialData.startDate ? new Date(initialData.startDate).toISOString().split("T")[0] : "");
        setEndDate(initialData.endDate ? new Date(initialData.endDate).toISOString().split("T")[0] : "");
        setApplyToSeparateProducts(initialData.applyToSeparateProducts);
        setComponents(initialData.components.map((comp, idx) => ({
          ...comp,
          tempId: comp.id || `temp-${idx}`
        })));
        setOriginalPrice(initialData.originalPrice);
        setSuggestedPrice(initialData.suggestedPrice);
        setCostPrice(initialData.costPrice);
        setProfitMargin(initialData.profitMargin ?? null);
      } else {
        resetForm();
      }
    }
  }, [isOpen, initialData]);
  const resetForm = () => {
    setSku("");
    setName("");
    setPromotionType("percentage");
    setPromotionValue("");
    setFinalPrice("");
    setIsActive(true);
    setIsScheduled(false);
    setStartDate("");
    setEndDate("");
    setApplyToSeparateProducts(true);
    setComponents([]);
    setOriginalPrice(0);
    setSuggestedPrice(0);
    setCostPrice(0);
    setProfitMargin(null);
  };
  reactExports.useEffect(() => {
    if (components.length > 0 && promotionType && promotionValue) {
      calculatePrices();
    }
  }, [components, promotionType, promotionValue]);
  const calculatePrices = async () => {
    if (components.length === 0) {
      setOriginalPrice(0);
      setSuggestedPrice(0);
      setCostPrice(0);
      setProfitMargin(null);
      return;
    }
    try {
      const value = parseFloat(promotionValue) || 0;
      const { originalPrice: original, suggestedPrice: suggested, costPrice: cost } = await promotionService.calculatePromotionPrices(components, promotionType, value);
      setOriginalPrice(original);
      setSuggestedPrice(suggested);
      setCostPrice(cost);
      const final = parseFloat(finalPrice) || suggested;
      if (final > 0) {
        setProfitMargin((final - cost) / final * 100);
      } else {
        setProfitMargin(null);
      }
    } catch (error) {
      console.error("Error calculating prices:", error);
    }
  };
  reactExports.useEffect(() => {
    if (finalPrice) {
      const final = parseFloat(finalPrice);
      if (final > 0 && costPrice > 0) {
        setProfitMargin((final - costPrice) / final * 100);
      }
    }
  }, [finalPrice, costPrice]);
  const handleAddComponent = () => {
    setActiveComponentIndex(components.length);
    setIsProductPickerOpen(true);
  };
  const handleProductSelected = async (product) => {
    if (activeComponentIndex === null) return;
    let productSku = product.sku || product.id;
    let saleOption = "package";
    if (product.fractionalEnabled && product.fractionalSubSkus && product.fractionalSubSkus.length > 0) {
      saleOption = "package";
    }
    const newComponent = {
      tempId: `temp-${Date.now()}`,
      productId: product.id,
      productSku,
      productName: product.name,
      quantity: 1,
      unitOfMeasure: product.unitOfMeasure,
      saleOption
    };
    const updatedComponents = activeComponentIndex === components.length ? [...components, newComponent] : components.map(
      (comp, idx) => idx === activeComponentIndex ? newComponent : comp
    );
    setComponents(updatedComponents);
    if (!name && updatedComponents.length === 1) {
      setName(product.name);
    }
    setIsProductPickerOpen(false);
    setActiveComponentIndex(null);
  };
  const handleRemoveComponent = (index) => {
    setComponents(components.filter((_, idx) => idx !== index));
  };
  const handleComponentChange = (index, field, value) => {
    setComponents(components.map(
      (comp, idx) => idx === index ? { ...comp, [field]: value } : comp
    ));
  };
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!sku.trim() || !name.trim()) {
      addNotification("SKU y nombre son requeridos", "error");
      return;
    }
    if (components.length === 0) {
      addNotification("Debe agregar al menos un componente", "error");
      return;
    }
    const value = parseFloat(promotionValue);
    if (isNaN(value) || value <= 0) {
      addNotification("El valor de la promoción debe ser mayor a 0", "error");
      return;
    }
    const final = parseFloat(finalPrice) || suggestedPrice;
    if (final <= 0) {
      addNotification("El precio final debe ser mayor a 0", "error");
      return;
    }
    if (isScheduled) {
      if (!startDate || !endDate) {
        addNotification("Las fechas son requeridas cuando la promoción está programada", "error");
        return;
      }
      if (new Date(startDate) >= new Date(endDate)) {
        addNotification("La fecha de inicio debe ser anterior a la fecha de fin", "error");
        return;
      }
    }
    setIsLoading(true);
    try {
      const componentsData = components.map((comp) => ({
        productId: comp.productId,
        productSku: comp.productSku,
        quantity: comp.quantity,
        unitOfMeasure: comp.unitOfMeasure,
        saleOption: comp.saleOption
      }));
      const promotionSku = sku || `PROMO-${Date.now()}`;
      const promotionName = name || (components.length > 0 ? components[0].productName : "Promoción sin nombre");
      if (initialData) {
        await promotionService.updatePromotion(initialData.id, {
          name: promotionName,
          isActive,
          promotionType,
          promotionValue: value,
          finalPrice: final,
          isScheduled,
          startDate: isScheduled && startDate ? new Date(startDate) : void 0,
          endDate: isScheduled && endDate ? new Date(endDate) : void 0,
          applyToSeparateProducts,
          components: componentsData
        });
      } else {
        await promotionService.createPromotion({
          sku: promotionSku,
          name: promotionName,
          promotionType,
          promotionValue: value,
          finalPrice: final,
          isScheduled,
          startDate: isScheduled && startDate ? new Date(startDate) : void 0,
          endDate: isScheduled && endDate ? new Date(endDate) : void 0,
          applyToSeparateProducts,
          components: componentsData
        });
      }
      onSubmit();
    } catch (error) {
      console.error("Error saving promotion:", error);
      addNotification(error.message || "Error al guardar promoción", "error");
    } finally {
      setIsLoading(false);
    }
  };
  if (!isOpen) return null;
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-slate-800 w-full max-w-4xl m-4 p-6 rounded-xl shadow-2xl border border-slate-700 max-h-[90vh] overflow-y-auto", onClick: (e) => e.stopPropagation(), children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between mb-6", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "text-xl font-bold text-slate-100", children: initialData ? "Editar Promoción" : "Nueva Promoción" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            type: "button",
            onClick: onClose,
            className: "text-slate-400 hover:text-slate-200 transition-colors",
            "aria-label": "Cerrar",
            children: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: "h-6 w-6", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M6 18L18 6M6 6l12 12" }) })
          }
        )
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("form", { onSubmit: handleSubmit, className: "space-y-6", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "block text-sm font-medium text-slate-300 mb-1", children: [
            "Nombre de la Promoción ",
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-xs text-slate-500", children: "(opcional)" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "input",
            {
              type: "text",
              value: name,
              onChange: (e) => setName(e.target.value),
              placeholder: components.length > 0 ? components[0].productName : "Se usará el nombre del primer producto",
              className: "w-full bg-slate-700 border border-slate-600 rounded-md px-3 py-2"
            }
          ),
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-slate-500 mt-1", children: "Si no se especifica, se usará el nombre del primer producto agregado" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-4", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-sm font-medium text-slate-300 mb-1", children: "Tipo de Promoción" }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "select",
              {
                value: promotionType,
                onChange: (e) => setPromotionType(e.target.value),
                className: "w-full bg-slate-700 border border-slate-600 rounded-md px-3 py-2",
                required: true,
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "2x1", children: "2x1 (Paga 1, lleva 2)" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "3x2", children: "3x2 (Paga 2, lleva 3)" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "percentage", children: "Porcentaje de descuento" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "fixed_discount", children: "Descuento fijo" })
                ]
              }
            )
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-sm font-medium text-slate-300 mb-1", children: promotionType === "percentage" ? "Porcentaje (%)" : promotionType === "fixed_discount" ? "Descuento ($)" : "Valor" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                type: "number",
                min: "0",
                step: promotionType === "percentage" ? "1" : "0.01",
                value: promotionValue,
                onChange: (e) => setPromotionValue(e.target.value),
                className: "w-full bg-slate-700 border border-slate-600 rounded-md px-3 py-2",
                required: true
              }
            )
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex justify-between items-center mb-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-sm font-medium text-slate-300", children: "Componentes" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "button",
              {
                type: "button",
                onClick: handleAddComponent,
                className: "px-3 py-1 text-sm bg-sky-600 text-white rounded-md hover:bg-sky-500",
                children: "+ Agregar Componente"
              }
            )
          ] }),
          components.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm text-slate-500 py-4 text-center", children: "No hay componentes. Agrega al menos uno." }) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "space-y-2", children: components.map((comp, idx) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-slate-700 rounded-lg p-4", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex justify-between items-start mb-2", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-slate-100 font-medium", children: comp.productName || comp.productSku }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-slate-400", children: comp.productSku })
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "button",
                {
                  type: "button",
                  onClick: () => handleRemoveComponent(idx),
                  className: "text-red-400 hover:text-red-300 text-sm",
                  children: "Eliminar"
                }
              )
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-3 gap-2", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-xs text-slate-400 mb-1", children: "Cantidad" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "input",
                  {
                    type: "number",
                    min: "0.01",
                    step: "0.01",
                    value: comp.quantity,
                    onChange: (e) => handleComponentChange(idx, "quantity", parseFloat(e.target.value) || 0),
                    className: "w-full bg-slate-800 border border-slate-600 rounded-md px-2 py-1 text-sm"
                  }
                )
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-xs text-slate-400 mb-1", children: "Unidad" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "input",
                  {
                    type: "text",
                    value: comp.unitOfMeasure,
                    disabled: true,
                    className: "w-full bg-slate-800 border border-slate-600 rounded-md px-2 py-1 text-sm disabled:opacity-50"
                  }
                )
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-xs text-slate-400 mb-1", children: "Opción de Venta" }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "select",
                  {
                    value: comp.saleOption || "package",
                    onChange: (e) => handleComponentChange(idx, "saleOption", e.target.value),
                    className: "w-full bg-slate-800 border border-slate-600 rounded-md px-2 py-1 text-sm",
                    children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "package", children: "Paquete Completo" }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "fraction", children: "Fracción" })
                    ]
                  }
                )
              ] })
            ] })
          ] }, comp.tempId)) })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-slate-900/50 rounded-lg", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-xs text-slate-400 mb-1", children: "Precio Original" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-lg font-semibold text-slate-300", children: formatCurrency(originalPrice) })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-xs text-slate-400 mb-1", children: "Precio Sugerido" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-lg font-semibold text-green-400", children: formatCurrency(suggestedPrice) })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-xs text-slate-400 mb-1", children: "Costo Total" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-lg font-semibold text-slate-300", children: formatCurrency(costPrice) })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-xs text-slate-400 mb-1", children: "Margen de Ganancia" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: `text-lg font-semibold ${profitMargin !== null && profitMargin >= 0 ? "text-green-400" : "text-red-400"}`, children: profitMargin !== null ? `${profitMargin.toFixed(1)}%` : "-" })
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-sm font-medium text-slate-300 mb-1", children: "Precio Final" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "input",
            {
              type: "number",
              min: "0",
              step: "0.01",
              value: finalPrice,
              onChange: (e) => setFinalPrice(e.target.value),
              className: "w-full bg-slate-700 border border-slate-600 rounded-md px-3 py-2",
              required: true
            }
          ),
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-slate-500 mt-1", children: "Puede modificar el precio sugerido" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-4", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-sm font-medium text-slate-300 mb-1", children: "Fecha de Inicio" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                type: "date",
                value: startDate,
                onChange: (e) => setStartDate(e.target.value),
                disabled: !isScheduled,
                className: "w-full bg-slate-700 border border-slate-600 rounded-md px-3 py-2 disabled:opacity-50"
              }
            )
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-sm font-medium text-slate-300 mb-1", children: "Fecha de Fin" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                type: "date",
                value: endDate,
                onChange: (e) => setEndDate(e.target.value),
                disabled: !isScheduled,
                className: "w-full bg-slate-700 border border-slate-600 rounded-md px-3 py-2 disabled:opacity-50"
              }
            )
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-2", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                type: "checkbox",
                id: "isScheduled",
                checked: isScheduled,
                onChange: (e) => setIsScheduled(e.target.checked),
                className: "form-checkbox bg-slate-600"
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsx("label", { htmlFor: "isScheduled", className: "text-sm text-slate-300", children: "Promoción programada (con calendario)" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                type: "checkbox",
                id: "isActive",
                checked: isActive,
                onChange: (e) => setIsActive(e.target.checked),
                className: "form-checkbox bg-slate-600"
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsx("label", { htmlFor: "isActive", className: "text-sm text-slate-300", children: "Promoción activa" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                type: "checkbox",
                id: "applyToSeparateProducts",
                checked: applyToSeparateProducts,
                onChange: (e) => setApplyToSeparateProducts(e.target.checked),
                className: "form-checkbox bg-slate-600"
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { htmlFor: "applyToSeparateProducts", className: "text-sm text-slate-300", children: [
              "Aplicar a productos separados en carrito",
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-xs text-slate-500 ml-1", children: "(detectar automáticamente cuando los productos están en el carrito)" })
            ] })
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex justify-end gap-4 pt-4", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              type: "button",
              onClick: onClose,
              className: "px-4 py-2 bg-slate-600 text-slate-200 font-semibold rounded-md hover:bg-slate-500 transition-colors",
              children: "Cancelar"
            }
          ),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              type: "submit",
              disabled: isLoading,
              className: "px-4 py-2 bg-sky-600 text-white font-semibold rounded-md hover:bg-sky-500 transition-colors disabled:opacity-50",
              children: isLoading ? "Guardando..." : "Guardar"
            }
          )
        ] })
      ] })
    ] }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      ProductPickerModal,
      {
        isOpen: isProductPickerOpen,
        onClose: () => {
          setIsProductPickerOpen(false);
          setActiveComponentIndex(null);
        },
        onSelect: handleProductSelected,
        title: "Seleccionar Producto",
        filter: (product) => !product.isSupply
      }
    )
  ] });
};

const PromotionsPage = () => {
  const { session } = useAuth();
  const { promotionService } = useServices();
  const { addNotification } = useNotification();
  const [promotions, setPromotions] = reactExports.useState([]);
  const [isLoading, setIsLoading] = reactExports.useState(true);
  const [isFormModalOpen, setIsFormModalOpen] = reactExports.useState(false);
  const [isDeleteModalOpen, setIsDeleteModalOpen] = reactExports.useState(false);
  const [editingPromotion, setEditingPromotion] = reactExports.useState(null);
  const [deletingPromotion, setDeletingPromotion] = reactExports.useState(null);
  const [searchTerm, setSearchTerm] = reactExports.useState("");
  reactExports.useEffect(() => {
    if (session?.organization?.id) {
      fetchPromotions();
    }
  }, [session?.organization?.id]);
  const fetchPromotions = async () => {
    if (!session?.organization?.id) return;
    setIsLoading(true);
    try {
      const data = await promotionService.getAllPromotions();
      setPromotions(data);
    } catch (error) {
      console.error("Error fetching promotions:", error);
      addNotification("Error al cargar promociones", "error");
    } finally {
      setIsLoading(false);
    }
  };
  const handleOpenAddModal = () => {
    setEditingPromotion(null);
    setIsFormModalOpen(true);
  };
  const handleOpenEditModal = (promotion) => {
    setEditingPromotion(promotion);
    setIsFormModalOpen(true);
  };
  const handleOpenDeleteModal = (promotion) => {
    setDeletingPromotion(promotion);
    setIsDeleteModalOpen(true);
  };
  const handleCloseModals = () => {
    setIsFormModalOpen(false);
    setIsDeleteModalOpen(false);
    setEditingPromotion(null);
    setDeletingPromotion(null);
  };
  const handleSavePromotion = async () => {
    await fetchPromotions();
    handleCloseModals();
    addNotification(editingPromotion ? "Promoción actualizada correctamente" : "Promoción creada correctamente", "success");
  };
  const handleDeletePromotion = async () => {
    if (!deletingPromotion) return;
    try {
      await promotionService.deletePromotion(deletingPromotion.id);
      await fetchPromotions();
      handleCloseModals();
      addNotification("Promoción eliminada correctamente", "success");
    } catch (error) {
      console.error("Error deleting promotion:", error);
      addNotification("Error al eliminar promoción", "error");
    }
  };
  const filteredPromotions = promotions.filter(
    (promotion) => promotion.name.toLowerCase().includes(searchTerm.toLowerCase()) || promotion.sku.toLowerCase().includes(searchTerm.toLowerCase())
  );
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "p-4 md:p-6", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex justify-between items-center mb-6", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { className: "text-2xl font-bold text-slate-100", children: "Promociones" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          onClick: handleOpenAddModal,
          className: "px-4 py-2 bg-sky-600 text-white font-semibold rounded-md hover:bg-sky-500 transition-colors",
          children: "+ Nueva Promoción"
        }
      )
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mb-4", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
      "input",
      {
        type: "text",
        placeholder: "Buscar promociones...",
        value: searchTerm,
        onChange: (e) => setSearchTerm(e.target.value),
        className: "w-full max-w-md bg-slate-700 border border-slate-600 rounded-md px-4 py-2 text-slate-100 placeholder-slate-400"
      }
    ) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      PromotionsTable,
      {
        promotions: filteredPromotions,
        isLoading,
        onEditPromotion: handleOpenEditModal,
        onDeletePromotion: handleOpenDeleteModal
      }
    ),
    isFormModalOpen && /* @__PURE__ */ jsxRuntimeExports.jsx(
      PromotionFormModal,
      {
        isOpen: isFormModalOpen,
        onClose: handleCloseModals,
        onSubmit: handleSavePromotion,
        initialData: editingPromotion
      }
    ),
    isDeleteModalOpen && deletingPromotion && /* @__PURE__ */ jsxRuntimeExports.jsx(
      ConfirmationModal,
      {
        isOpen: isDeleteModalOpen,
        onClose: handleCloseModals,
        onConfirm: handleDeletePromotion,
        title: "Eliminar Promoción",
        message: `¿Estás seguro de que deseas eliminar la promoción "${deletingPromotion.name}"? Esta acción no se puede deshacer.`
      }
    )
  ] });
};

export { PromotionsPage as default };
