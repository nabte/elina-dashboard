import"./auth-CBpGm6GX.js";let m=!1;document.addEventListener("auth:ready",async({detail:a})=>{if(!a||!a.session){window.location.href="/";return}if(m)return;const{data:t,error:l}=await window.auth.sb.from("profiles").select("role").eq("id",a.session.user.id).single();if(l||!t||t.role!=="superadmin"){alert("Acceso denegado. No tienes permisos de superadministrador."),window.location.href="/dashboard.html";return}m=!0,document.getElementById("loader").classList.add("hidden"),document.getElementById("app-container").classList.remove("hidden"),b()});function b(){f(),p(),g()}function g(){var a;document.getElementById("plans-container").addEventListener("submit",t=>{t.target.classList.contains("plan-form")&&(t.preventDefault(),v(t.target))}),(a=document.getElementById("mobile-menu-btn"))==null||a.addEventListener("click",()=>{const t=document.getElementById("main-sidebar");t==null||t.classList.toggle("hidden")}),document.getElementById("user-search").addEventListener("input",t=>{const l=t.target.value.toLowerCase();document.querySelectorAll("#users-table-body tr").forEach(e=>{const n=e.dataset.email.toLowerCase();e.style.display=n.includes(l)?"":"none"})}),document.getElementById("users-table-body").addEventListener("click",t=>{if(t.target.closest(".change-plan-btn")){const l=t.target.closest(".change-plan-btn");y(l.dataset.userId,l.dataset.userEmail,l.dataset.currentPlan)}if(t.target.closest(".impersonate-btn")){const l=t.target.closest(".impersonate-btn");t.stopPropagation(),_(l.dataset.userId)}}),document.getElementById("cancel-change-plan-btn").addEventListener("click",()=>{document.getElementById("change-plan-modal").classList.add("hidden")}),document.getElementById("confirm-change-plan-btn").addEventListener("click",h)}async function f(){const a=document.getElementById("plans-container"),{data:t,error:l}=await window.auth.sb.from("plans").select("*");if(l){a.innerHTML='<p class="text-red-400">Error al cargar planes.</p>';return}a.innerHTML=t.map(e=>{var n,s,i,o,r,d,u,c;return`
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <select class="features-follow-ups mt-1 w-full bg-slate-700 border-slate-600 rounded p-2">
                        ...
                        <option value="false" ${(o=e.features)!=null&&o.follow_ups?"":"selected"}>No</option>
                    </select>
                </div>
                <div>
                    ...
                    <select class="features-designer-ai mt-1 w-full bg-slate-700 border-slate-600 rounded p-2">
                        ...
                        <option value="false" ${(d=e.features)!=null&&d.designer_ai?"":"selected"}>No</option>
                    </select>
                </div>
                <div>
