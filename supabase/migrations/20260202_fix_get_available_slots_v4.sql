...
BEGIN
    ...
    IF NOT FOUND THEN
         RETURN jsonb_build_object('available_slots', '[]'::jsonb, 'error', 'No hours configured for this day (' || v_day_of_week || ')');
    END IF;
    ...
    WHILE v_slot_start + (v_duration || ' minutes')::interval <= v_date_end LOOP
         ...
         IF v_is_available THEN
             FOR v_meeting IN 
                ...
                WHERE user_id = p_user_id 
                AND status IN ('confirmed', 'pending')
                -- Filter by date relative to user timezone to be efficient
                -- Meeting must start before slot ends (plus buffer)
                -- And end after slot starts (minus buffer)
                AND start_time < (v_slot_end + (v_buffer || ' minutes')::interval) AT TIME ZONE v_timezone
                AND end_time > (v_slot_start - (v_buffer || ' minutes')::interval) AT TIME ZONE v_timezone
             LOOP
                -- If we found ANY meeting that overlaps, the slot is taken
                v_is_available := FALSE;
                EXIT;
             END LOOP;
         END IF;
