...
Deno.serve(async (req) => {
    ...
    try {
        ...
        flowDefinition = {
            ...
            mode: flowData.flow_data?.mode || 'step_by_step', // NEW: Flow mode
            steps: Array.isArray(flowData.flow_data)
                ? flowData.flow_data
                : (flowData.flow_data?.steps || []),
            variables: {},
            get_all_config: flowData.flow_data?.get_all_config // NEW: Get All configuration
        };

        console.log(`[SmartFlowEngine] Flow mode: ${flowDefinition.mode}`);
        ...
        if (isNewRequest) {
            ...
            // ========== GET ALL MODE - FIRST MESSAGE ==========
            // If flow is in "get_all" mode, try to extract fields from the initial message
            if (flowDefinition.mode === 'get_all' && input_text) {
                console.log('[SmartFlowEngine] 🤖 GET_ALL MODE (New Request) - Extracting from first message');

                const { GetAllHandler } = await import('./core/GetAllHandler.ts');
                const getAllHandler = new GetAllHandler();

                // Extract images if present
                const messageImages: string[] = [];
                if (body.data?.message?.imageMessage?.url) {
                    messageImages.push(body.data.message.imageMessage.url);
                }

                // Extract all fields from initial message
                ...
                console.log('[SmartFlowEngine] GET_ALL (new) extraction result:', extractionResult);

                // Merge extracted variables into state
                state.variables = {
                    ...state.variables,
                    ...extractionResult.extracted
                };

                // If not all fields collected, send question and pause immediately
                if (extractionResult.completion_percentage < 100 && extractionResult.next_question) {
                    console.log(`[SmartFlowEngine] 📝 GET_ALL (new): ${extractionResult.completion_percentage}% complete on first message`);
                        ...
                        try {
                            await fetch(`${evolutionApiUrl}/message/sendText/${instance}`, {
                                ...
                                body: JSON.stringify({
                                    ...
                                    text: extractionResult.next_question
                                })
                            });
                            console.log('[SmartFlowEngine] GET_ALL initial question sent');
                        } catch (error) {
                            console.error('[SmartFlowEngine] Error sending GET_ALL question:', error);
                        }
                    }
                    ...
                    return new Response(JSON.stringify({
                        ...
                        completion_percentage: extractionResult.completion_percentage,
                        missing_fields: extractionResult.missing_fields,
                        question_sent: extractionResult.next_question
                    }), {
                        headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
                        status: 200
                    });
                ...
                console.log('[SmartFlowEngine] ✅ GET_ALL (new): All fields collected from first message! Continuing...');
            }
        } else {
            ...
            if (existingState) {
                ...
                // ========== GET ALL MODE PROCESSING ==========
                // If flow is in "get_all" mode, use intelligent multi-field extraction
                if (flowDefinition.mode === 'get_all' && input_text) {
                    console.log('[SmartFlowEngine] 🤖 GET_ALL MODE - Processing multi-field extraction');
                    ...
                    // Extract all fields from current message
                    const extractionResult = await getAllHandler.extractAllFields(
                        input_text,
                        flowDefinition,
                        state,
                    ...
                    console.log('[SmartFlowEngine] GET_ALL extraction result:', extractionResult);
                    ...
                    // If all fields collected, continue with flow execution
                    if (extractionResult.completion_percentage === 100) {
                        console.log('[SmartFlowEngine] ✅ GET_ALL: All fields collected! Continuing flow...');
                        state.status = 'active';
                        // The flow executor will handle the rest
                    }
                    // If still missing fields, send next question and pause
                    else if (extractionResult.next_question) {
                        console.log(`[SmartFlowEngine] 📝 GET_ALL: ${extractionResult.completion_percentage}% complete. Asking for missing fields...`);
                        ...
                        if (evolutionApiUrl && evolutionApiKey) {
                            try {
                                ...
                                console.log('[SmartFlowEngine] GET_ALL question sent successfully');
                            } catch (error) {
                                console.error('[SmartFlowEngine] Error sending GET_ALL question:', error);
                            }
                        }
