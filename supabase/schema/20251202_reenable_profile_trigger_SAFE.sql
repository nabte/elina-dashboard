...
-- VERSIÓN SEGURA - Solo habilita si ya existe, no intenta crear
-- ============================================
...
-- Paso 2: Re-habilitar el trigger (solo si ya existe)
-- Si el trigger no existe, este comando no hará nada pero no fallará
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'on_auth_user_created') THEN
        ALTER TABLE auth.users ENABLE TRIGGER on_auth_user_created;
        RAISE NOTICE 'Trigger on_auth_user_created habilitado';
    ELSE
        RAISE NOTICE 'Trigger on_auth_user_created no existe. Necesitas crearlo manualmente o usar el SQL original.';
    END IF;
END $$;

-- Paso 3: Verificar que la función handle_new_user existe
SELECT 
    proname as function_name,
    CASE 
        WHEN proname = 'handle_new_user' THEN 'Existe ✅'
        ELSE 'No existe ❌'
    END as status
FROM pg_proc
WHERE proname = 'handle_new_user';

-- Paso 4: Crear/actualizar la función handle_new_user
-- (Esta sí puedes crearla porque es en el esquema public)
...
-- Paso 5: Verificar estado final del trigger
SELECT 
    tgname as trigger_name,
    CASE tgenabled
        WHEN 'A' THEN 'Enabled ✅'
