...
class DashboardApp {
    ...
    async checkAppointmentsEnabled() {
        try {
            ...
            const { data: settings, error } = await auth.sb
    ...
    async switchPanel(targetId, options = {}) { // CORRECCIÓN: Lógica para modo pantalla completa de Kanban
        ...
        const groupConfig = {
            ...
            'ai-behavior': {
                tabs: [
                    { id: 'prompt-training', label: 'Entrenamiento de Prompt', default: true },
...
class Wizard {
    ...
    async loadExistingCompanySettings() {
        try { // CORRECCIÓN: Verificar si el formulario existe
            ...
            const { data, error } = await window.auth.sb.from('profiles').select('work_start_hour, work_end_hour, company_description, website, social_media, branding_settings, contact_phone, organization_id').single();
